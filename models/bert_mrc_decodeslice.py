...
class BertTagger(nn.Module):
    def __init__(self, config, num_labels=4,device = 'cpu',load_best =False,dropout_sample=8):
        ...
        self.device = device
        self.dropout_sample = dropout_sample
        # self.position_embedding = PositionalEncoding(self.config.hidden_size,max_len=self.config.max_seq_length)
        if not config.start_end:
            ...
            if load_best:
                ...
                self.bert = BertForQuestionAnswering.from_pretrained(config.bert_model)
       # print("in bert_mrc : device= ",device)
        self.to(device)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, \
        ...
        if self.config.start_end:
            ...
            position_ids = None
            # if self.config.sincos_position_embedding:
            #     position_ids = torch.tensor([self.position_embedding(input_id).tolist() for input_id in input_ids])
            # print('position_ids = {}'.format(position_ids))

            last_bert_layer, pooled_output = self.bert(input_ids,attention_mask, token_type_ids,position_ids )
            # print('in bert_mrc, position_ids[0] = {},last_bert_layer[0] = {}'.format(position_ids[0],last_bert_layer[0]))
            ...
            if self.config.use_crf:
             ...
             # pad tags if using batch-CRF decoder
                for logits in logit_samples:
                    ...
                    # print(labels[0].numpy(),context_lengths[0][0].item())
                    # for i in range(len(context_lengths)):
                        # try:
                        # print(labels[i,2 + context_lengths[i][0].item():])
                        # labels_lst.append(labels[i,2 + context_lengths[i][0].item():].detach().numpy().tolist())
                        # logits_lst.append(logits[i,2 + context_lengths[i][0].item():,:].detach().numpy().tolist())
                        # length_lst.append( context_lengths[i][1].item())
                            # labels_lst.append(labels[ix].detach().numpy().tolist()[2 + context_lengths[ix][0].item():,:])
                            # logits_lst.append(logits[ix].detach().numpy().tolist()[2 + context_lengths[ix][0].item():,:])
                            # length_lst.append( context_lengths[ix][1].item())
                        # except:
                        #     print(i)
                    # for qa,label,length in zip(logits,labels,context_lengths):
                    # labels = torch.tensor(labels_lst,dtype=torch.long)
                    # logits = torch.tensor(logits_lst)

                    # tags, _ = pad_tensors(labels)

                    # forward_score = self._forward_alg(logits, context_lengths.detach().numpy().tolist())
                    # gold_score = self._score_sentence(logits, labels, context_lengths.detach().numpy().tolist())
                    #
                ...
                if self.config.use_crf:
                    ...
                    logit = logit_samples[0].squeeze(-1)[:,1:,:]
                    attention_mask = attention_mask.to(torch.uint8)[:,1:]
                    return self.crf_model.decode(logit,attention_mask)
                else:
                    if self.config.start_end:
                        print(start_score[0],'\n',end_score[0])
