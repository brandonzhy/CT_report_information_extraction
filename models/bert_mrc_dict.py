...
class BertTagger(nn.Module):
    def __init__(self, config,device, num_labels=4,load_best =False,dropout_sample=8):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, \
        ...
        self.zero_grad()
        lengths_context = context_lengths['context'].tolist()
        lengths_question = context_lengths['question'].tolist()
            ...
            if self.config.use_crf:
             ...
             # pad tags if using batch-CRF decoder
                for logits in logit_samples:
                   ...
                   # print("len(labels) ={}, len(logits) ={}. len(context_lengths) ={}".format(len(labels),len(logits), len(context_lengths)))
                    assert  len(labels) == len(logits) == len(lengths_context)
                    if class_weight:
                ...
                if type(labels)==tuple:
                    ...
                    if loss_sign == "ce":
                        ...
                        for logits in logit_samples:
                            ...
                            for qa,label,length_context,length_question,qa_type in zip(logits,labels,lengths_context,lengths_question,question_type):
                                # print('in bert_mrc line 93: ',length)
                                # qa = torch.argmax(qa,dim = -1)
                                # 注意，这里传入CrossEntropyLoss的两个参数，第一个是predict:(seq_len,label_num)，第而个是label：（seq_len,)
                                # 传人（seq_len,)的会报错（rossEntropyLoss内部会有argmax操作）
                                ...
                                if self.config.question_first:
                                    pre_index = 2 + length_question
                                else:
                                    pre_index = 1
                               #question before context
                               # pre_index = 2 + length[0]
                                if class_weight:
                                    loss += class_weight[qa_type.item()]*loss_fct(qa[pre_index : pre_index + length_context], label[pre_index: pre_index + length_context])
                                else:
                                    loss += loss_fct(qa[pre_index : pre_index + length_context], label[pre_index: pre_index + length_context])
                ...
                if self.config.use_crf:
                   # print('use viterbi_code')
                    ...
                    #     tags.append(tag_seq)
                    return self.crf_model.decode(logit_samples[0].squeeze(-1))
