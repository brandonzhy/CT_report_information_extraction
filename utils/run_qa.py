...
def args_parser():
    ...
    parser.add_argument("--bert_model",
                        default="./pretrained_model/chinese-bert_chinese_wwm_pytorch", type=str, required=True,
    ...
    parser.add_argument("--num_train_epochs", default=46, type=int, required=True,)
    parser.add_argument('--use_crf', type=bool, default=False)
    parser.add_argument("--train_batch_size", default=32,  required=True,type=int)
    ...
    parser.add_argument('--decode_mask', type=bool, default=False)
    parser.add_argument('--nlayers', type=int, default=2)
    parser.add_argument('--use_rnn', type=bool, default=False)
    parser.add_argument('--sincos_position_embedding', type=bool, default=False)
    parser.add_argument('--question_first', type=bool, default=False,
                       help="If true, concatenate question before question " )
    parser.add_argument('--start_end', type=bool, default=False,
...
def get_features(config, logger):
    out_train_input_ids = os.path.join(config.data_dir, 'train_qa_modifiedLymph_artificial_question_neg_format_input_ids.npy')
    out_test_input_ids = os.path.join(config.data_dir, 'test_qa_modifiedLymph_artificial_question_neg_format_input_ids.npy')
    out_valid_input_ids = os.path.join(config.data_dir, 'valid_qa_modifiedLymph_artificial_question_neg_format_input_ids.npy')

    out_train_input_masks = os.path.join(config.data_dir, 'train_qa_modifiedLymph_artificial_question_neg_format_input_masks.npy')
    out_test_input_masks = os.path.join(config.data_dir, 'test_qa_modifiedLymph_artificial_question_neg_format_input_masks.npy')
    out_valid_input_masks = os.path.join(config.data_dir, 'valid_qa_modifiedLymph_artificial_question_neg_format_input_masks.npy')

    out_train_question_types = os.path.join(config.data_dir, 'train_qa_modifiedLymph_artificial_question_neg_format_question_types.npy')
    out_test_question_types = os.path.join(config.data_dir, 'test_qa_modifiedLymph_artificial_question_neg_format_question_types.npy')
    out_valid_question_types = os.path.join(config.data_dir, 'valid_qa_modifiedLymph_artificial_question_neg_format_question_types.npy')

    out_train_segment_ids = os.path.join(config.data_dir, 'train_qa_modifiedLymph_artificial_question_neg_format_segment_ids.npy')
    out_test_segment_ids = os.path.join(config.data_dir, 'test_qa_modifiedLymph_artificial_question_neg_format_segment_ids.npy')
    out_valid_segment_ids = os.path.join(config.data_dir, 'valid_qa_modifiedLymph_artificial_question_neg_format_segment_ids.npy')

    out_train_labels = os.path.join(config.data_dir, 'train_qa_modifiedLymph_artificial_question_neg_format_labels.npy')
    out_test_labels = os.path.join(config.data_dir, 'test_qa_modifiedLymph_artificial_question_neg_format_labels.npy')
    out_valid_labels = os.path.join(config.data_dir, 'valid_qa_modifiedLymph_artificial_question_neg_format_labels.npy')

    out_train_context_lengths = os.path.join(config.data_dir, 'train_qa_modifiedLymph_artificial_question_neg_format_context_lengths.npy')
    out_test_context_lengths = os.path.join(config.data_dir, 'test_qa_modifiedLymph_artificial_question_neg_format_context_lengths.npy')
    out_valid_context_lengths = os.path.join(config.data_dir, 'valid_qa_modifiedLymph_artificial_question_neg_format_context_lengths.npy')
...
def load_model(config, num_train_steps, num_labels, device):
        ...
        for text in data_text:
            ...
            if len(text)>0 and ('肝' not in text) and ('胃' not in text) and ('甲状腺'not in text):
                ...
                # consider all head_question as a batch
                    logits = best_model(input_ids, segment_ids, input_mask, labels=None, context_lengths=context_lengths)
                            ...
                            if question_turn[subject_type]!=[]:
                               ...
                               # subject_type = head_entity['type']
                                ...
                                for head_entity_token in head_entity_token_lst:
                                    tail_entity_dic_list = get_tail_entities(text,config,head_entity_token, device,subject_type,best_model,tokenizer)
...
def main():
    ...
    if config.do_predict:
        model, optimizer = load_model(config,0, len(question_dic.keys()), device)
        predict(model, config, device)
        #cal_triple_res(os.path.join(config.data_dir, 'test_data_clean.json'),os.path.join(config.output_dir, 'test_predict.json'))
